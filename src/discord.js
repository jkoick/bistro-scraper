...
export class DiscordNotifier {
  ...
  async sendMessage(messageData) {
    try {
      ...
      if (error.response) {
        throw new Error(
          `Discord API error: ${error.response.status} - ${
      ...
      embeds: [
        {
          ...
          description: `An error occurred while scraping menus${
            context ? ` (${context})` : ""
          }`,
          fields: [
            {
  ...
  async cleanupScreenshots(screenshotPath) {
    try {
      this.logger.info(
        "Cleaning up screenshots after successful message sending"
      );
      ...
      this.logger.info(
        `Successfully deleted ${deletedCount}/${screenshotFiles.length} screenshots`
      );
    } catch (error) {
